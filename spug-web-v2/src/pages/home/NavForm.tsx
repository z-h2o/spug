...
const NavForm: React.FC<NavFormProps> = ({ record: initialRecord, onCancel, onOk }) => {
  const [form] = Form.useForm();
  const [loading, setLoading] = useState(false);
  const [record, setRecord] = useState<NavRecord>(initialRecord);
  const [fileList, setFileList] = useState<any[]>([]);

  useEffect(() => {
    if (initialRecord.logo) {
      setFileList([{ uid: 0, thumbUrl: initialRecord.logo }]);
    }
  }, [initialRecord]);
  ...
  function changeLink(e: React.ChangeEvent<HTMLInputElement>, index: number, key: keyof NavLink) {
          ...
          {record.links.map((item, index) => (
            <div key={index} style={{ display: 'flex', alignItems: 'center', marginBottom: 12 }}>
              <Form.Item style={{ display: 'inline-block', margin: 0, width: 100 }}>
                <Input 
                  value={item.name} 
                  onChange={e => changeLink(e, index, 'name')} 
                  placeholder="链接名称" 
                />
              </Form.Item>
              <Form.Item style={{ display: 'inline-block', width: 210, margin: '0 8px' }}>
                <Input 
                  value={item.url} 
                  onChange={e => changeLink(e, index, 'url')} 
                  placeholder="请输入链接地址" 
                />
              </Form.Item>
