...
const Notice: React.FC = () => {
  ...
  const [record, setRecord] = useState<NoticeItem | null>(null);
  ...
  function showForm(info: Partial<NoticeItem>) {
    setRecord(info as NoticeItem);
    setTimeout(() => form.resetFields());
  }

  function handleSort(e: React.MouseEvent, info: NoticeItem, sort: string) {
    e.stopPropagation();
  ...
  function handleRead() {
    if (notice && !notice.read_ids.includes(id)) {
  ...
  function handleDelete(item: NoticeItem) {
    Modal.confirm({
      title: '操作确认',
  ...
  return (
    <>
      ...
      >
        {isEdit ? (
          <List>
            <div className={styles.add} onClick={() => showForm({})}>
              <PlusOutlined />新建公告
            </div>
            {records.map(item => (
          ...
          <List>
            {records.map(item => (
              ...
              >
                {!item.read_ids.includes(id) && (
                  <SoundOutlined style={{ color: '#ff4d4f', marginRight: 4 }} />
                )}
      ...
      >
        ...
        >
          ...
          <Form.Item required name="content" label="公告内容">
