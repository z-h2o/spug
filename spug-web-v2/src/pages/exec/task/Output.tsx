...
const Output: React.FC<OutputProps> = ({ onBack }) => {
  const {
    token,
    getItems,
    getCounter,
  ...
  useEffect(() => {
    if (activeId && terminalRef.current) {
      ...
      // 连接WebSocket
      if (token) {
        const wsUrl = `ws://127.0.0.1:9001/api/ws/exec/${token}/?id=${activeId}`;
      ...
      <div className={styles.content}>
        <div className={styles.sidebar}>
          <div className={styles.summary}>
            <Space>
              <Tag color="orange">连接中 {counter['0']}</Tag>
        ...
        <div className={styles.terminal}>
          <Card
            size="small"
            title={
              items.find(([id]) => id === activeId)?.[1]?.title || '执行结果'
            }
            bodyStyle={{ padding: 0 }}
          >
